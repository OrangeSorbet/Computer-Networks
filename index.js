  ...
  const LAYER_INFO = [
    ...
    },
    {
      ...
      functions: [
        ...
        ['Congestion Control','Prevents the network itself from being overwhelmed by too many packets in transit simultaneously, causing router queue overflow and packet loss','When aggregate traffic across multiple flows exceeds the capacity of a bottleneck link or router','Routers signal congestion via ICMP Source Quench (deprecated), ECN marks (setting CE bits in IP header), or implicit signals (packet drop); senders react by reducing transmission rate; AQM algorithms (RED, CoDel) drop packets proactively before queues fill completely','Without network-layer congestion control, all router queues fill to capacity, latency spikes, packets are dropped en masse, and the network enters congestion collapse where throughput approaches zero despite links being busy','Solves: router queue overflow and network-wide throughput collapse under high load'],
        ['Address Transformation (NAT)','Translates private (RFC 1918) IP addresses to one or more public IP addresses at the network boundary, allowing many hosts to share a single public IP','At every router or gateway performing NAT, on every packet crossing the private-to-public boundary','NAT router maintains a translation table mapping (private IP, private port) → (public IP, public port); rewrites src IP+port on outgoing packets and dst IP+port on incoming reply packets; NAPT (Port Address Translation) is the most common form allowing thousands of hosts behind one public IP','Without NAT, every device would require a globally unique public IPv4 address — the exhausted IPv4 address space would have made internet growth impossible; NAT also provides implicit firewall behaviour by blocking unsolicited inbound connections','Solves: IPv4 address exhaustion; enables private networks to access the internet with minimal public IP allocation'],
