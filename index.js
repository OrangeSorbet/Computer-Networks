  ...
  const LAYER_INFO = [
    {
      ...
      protocols: [
        ['<strong>Application Protocols</strong>', '<strong>HTTP, FTP, SMTP, DNS, DHCP, SNMP, SMPP, TFTP</strong>', '<strong>Protocols operating at the Application Layer</strong>', '<strong>Direct user/app interaction</strong>', '<strong>Define message formats, commands, and state machines for specific services</strong>', '<strong>HTTP · FTP · SMTP · DNS · DHCP · SNMP · SMPP · TFTP</strong>'],
        ...
        ['POP3','Post Office Protocol version 3','Retrieves email from a mail server to a local client — download and optionally delete from server','When a mail client fetches incoming email; port 110 (plaintext), port 995 (POP3S over TLS)','Client connects to TCP 110; authenticates with USER/PASS; issues STAT (mailbox size), LIST (message list), RETR n (download message n), DELE n (mark for deletion), QUIT (commit deletions and disconnect)','Without POP3, mail clients could not download messages from remote servers for offline reading'],
        ['POP (POP2)','Post Office Protocol version 2','Earlier version of POP — predecessor to POP3; largely obsolete','Legacy mail systems; port 109','Similar command flow to POP3 but different command syntax and response format; superseded by POP3 which added APOP authentication and better error handling','Historically significant as the first standardised mail retrieval protocol; understanding POP2 explains why POP3 was redesigned'],
        ['IMAP','Internet Message Access Protocol','Retrieves and manages email on the server — messages stay on server; supports folders, flags, and partial fetch','When a mail client needs to sync with a server across multiple devices; port 143 (plaintext), port 993 (IMAPS over TLS)','Client connects to TCP 143; authenticates; issues SELECT INBOX (open folder), FETCH 1 BODY[] (download message), STORE 1 +FLAGS (\\Seen) (mark read), SEARCH (query messages), EXPUNGE (permanently delete flagged messages); server maintains state across sessions','Without IMAP, users accessing email from multiple devices (phone, laptop, webmail) would have inconsistent mailbox state — POP3 deletes messages after download leaving other devices empty'],
        ['TFTP','Trivial File Transfer Protocol','Simplified file transfer with no authentication, no directory listing, and minimal overhead — uses UDP for speed','Network booting (PXE), firmware updates, router config transfer, diskless workstation boot; port 69 (UDP)','Client sends RRQ (Read Request) or WRQ (Write Request) to UDP 69; server responds with DATA blocks (512 bytes each); client ACKs each block; transfer ends when a block < 512 bytes is received; no login, no ls command, no error recovery beyond simple retransmit','Without TFTP, network devices could not boot or receive firmware images over a network — its simplicity makes it ideal for embedded systems with no TCP stack'],
      ]
    },
    {
      ...
      protocols: [
        ['<strong>Presentation Protocols</strong>', '<strong>TLS, SSL, MIME, ASCII, UTF-8, JPEG, MPEG, JSON, XML, Protobuf, gzip, Brotli</strong>', '<strong>Protocols operating at the Presentation Layer</strong>', '<strong>Translation, encryption, compression, serialisation</strong>', '<strong>Convert data formats, encrypt/decrypt, compress/decompress between application and network</strong>', '<strong>TLS · SSL · MIME · ASCII · UTF-8 · JPEG · MPEG · JSON · XML · Protobuf · gzip · Brotli</strong>'],
    ...
    },
    {
      ...
      protocols: [
        ['<strong>Session Protocols</strong>', '<strong>NetBIOS, RPC, SIP, PPTP, H.245, H.323, PAP, CHAP, NFS, ZIP</strong>', '<strong>Protocols operating at the Session Layer</strong>', '<strong>Session establishment, maintenance, synchronisation, termination</strong>', '<strong>Manage dialogues between applications: open sessions, checkpoint transfers, close sessions</strong>', '<strong>NetBIOS · RPC · SIP · PPTP · H.245 · H.323 · PAP · CHAP · NFS · ZIP</strong>'],
    ...
    },
    {
      ...
      functions: [
        ...
        ['Service Point Addressing','Identifies the specific process or application on a host that should send or receive data, using port numbers as service access points — ensuring data reaches not just the correct host but the correct process on that host','On every segment sent or received, whenever multiple processes share the same IP address','Each segment carries a source port and destination port; the OS maintains a socket table mapping (protocol, local IP, local port, remote IP, remote port) to a process; incoming segments are matched against this table to find the correct socket queue','Without service point addressing, the OS could deliver a packet to the correct host but would have no mechanism to determine whether it belongs to the web browser, mail client, SSH daemon, or any other running process — all processes would have to share one undifferentiated byte stream','Solves: delivering data to the correct process on a multi-application host rather than just the correct machine'],
        ['Connection Control','Determines whether communication between two processes is connection-oriented (a logical connection is established, maintained, and terminated) or connectionless (each segment is independent with no prior setup)','At the start of every transport-layer communication session','TCP (connection-oriented): 3-way handshake (SYN→SYN-ACK→ACK) establishes shared state (seq numbers, window sizes) before any data flows; FIN/FIN-ACK/ACK tears it down gracefully. UDP (connectionless): sender fires datagrams with no handshake, no state, no teardown — each datagram is routed independently','Without connection control, there is no mechanism to establish shared state between sender and receiver — reliable ordered delivery is impossible without the sequence number and acknowledgement agreement that the handshake establishes; conversely, forcing a handshake on every DNS query or VoIP packet would add unacceptable latency','Solves: establishing the shared context required for reliable delivery while allowing connectionless mode for latency-sensitive applications'],
        ...
        ['Error Control','Ensures that lost, corrupted, duplicated, or out-of-order segments are detected and corrected at the transport layer through retransmission — providing end-to-end reliable delivery between processes','During every TCP connection whenever a segment is lost, corrupted, or unacknowledged within the retransmission timeout','TCP sender starts a retransmission timer for every unACKed segment; if timer expires, segment is retransmitted; duplicate ACKs (3× same ACK) trigger fast retransmit without waiting for timeout; SACK (Selective ACK) tells sender exactly which segments arrived so only missing ones are retransmitted; sequence numbers detect duplicates and out-of-order delivery','Without error control at the transport layer, any packet lost in the network would silently corrupt the data stream — a file transfer with 0.1% packet loss would produce a corrupted file; every application would have to implement its own retransmission logic, guaranteed to be inconsistent and buggy across implementations','Solves: silent data loss, corruption, duplication, and reordering between communicating processes'],
      ...
      protocols: [
        ['<strong>Transport Protocols</strong>', '<strong>TCP, UDP, QUIC, SCTP, DCCP, SPX</strong>', '<strong>Protocols operating at the Transport Layer</strong>', '<strong>Segmentation, reliability, flow control, multiplexing</strong>', '<strong>Provide end-to-end process-to-process communication with port-based demultiplexing</strong>', '<strong>TCP · UDP · QUIC · SCTP · DCCP · SPX</strong>'],
    ...
    },
    {
      ...
      functions: [
        ['Encapsulation and De-encapsulation','Encapsulates segments on the sender side and de-encapsulates frames on the receiver side','On every packet sent and received','Sender wraps transport segment with IP header forming a packet; receiver strips IP header and passes payload up','Without encapsulation there is no packet structure and no way to carry addressing or routing metadata',''],
        ['IP Addressing','The IP addresses of the sender and receiver are stored in the packet header','On every packet','32-bit (IPv4) or 128-bit (IPv6) src and dst addresses placed in IP header; routers use dst address for forwarding','Without IP addressing routers cannot determine where to send a packet',''],
        ['Connectionless Service','No dedicated path is established between devices in advance for communication; each packet routed independently','On every packet in IP networks','Each router makes an independent forwarding decision per packet; packets may take different routes to the same destination','Without connectionless service the network would require per-flow state at every router — unscalable at internet size',''],
        ['Routing Decision','Finds the best route to send packets from source to destination','At every router for every packet','Routing protocols (OSPF, BGP) build routing tables; longest-prefix match selects next hop; TTL prevents loops','Without routing decisions packets cannot traverse multiple networks',''],
      ],
      design_issues: [
        ['Store-and-Forward Packet Switching','Routers receive the complete packet, store it temporarily, then forward to the next hop','Ensures error checking and proper routing before transmission','Adds latency proportional to packet size × hops; buffers can overflow under load'],
        ['Services Provided to the Transport Layer','Must provide reliable and efficient logical communication between source and destination','Defines whether service is connection-oriented (virtual circuit) or connectionless (datagram)','Connection-oriented gives QoS guarantees; connectionless is resilient and scalable — IP chose connectionless'],
        ['Implementation of Connectionless Service (Datagram Model)','Each packet treated independently; may follow different routes to reach destination','Example: Internet Protocol (IP)','Simple and resilient; no setup overhead; reliability delegated to transport layer (TCP)'],
        ['Implementation of Connection-Oriented Service (Virtual Circuit Model)','A logical path is established before transmission; all packets follow the same path','Resources can be reserved for better Quality of Service (QoS)','Predictable QoS but requires signalling and per-circuit state at every router on the path'],
        ['Routing Algorithms','Determines the best path for packet delivery from source to destination','Can be static or dynamic depending on network conditions','Static: simple but cannot adapt to failures; Dynamic (OSPF, BGP): adapts to topology changes automatically'],
        ['Congestion Control','Mechanisms to prevent network overload when too many packets are present','Proper congestion control improves throughput and reduces packet loss','ECN marking, AQM algorithms (RED, CoDel), sender-side rate reduction on congestion signals'],
        ['Quality of Service (QoS)','Must support bandwidth, delay, jitter, and reliability for real-time applications such as video streaming and VoIP','Best-effort IP treats all packets equally; differentiation requires extra mechanisms','DSCP marking + DiffServ queuing prioritises latency-sensitive traffic over bulk transfers'],
        ['Internetworking','Connecting multiple heterogeneous networks into a single unified network','Protocols like IP enable communication across different network technologies','IP provides a universal L3 abstracting over all L2 differences; routers bridge between link types'],
        ['Addressing','Logical addressing (IP) used to uniquely identify devices across networks','Ensures proper packet delivery to the correct destination','IPv4: 32-bit; IPv6: 128-bit; hierarchical allocation enables routing aggregation via CIDR'],
        ['Fragmentation and Reassembly','Large packets divided into smaller fragments to pass through networks with smaller MTU','Fragments reassembled at the destination','IPv4: routers or source may fragment; IPv6: source only via PMTUD; DF+ICMP enables Path MTU Discovery'],
      ],
      protocols: [
        ['<strong>Network Layer Protocols (Host & Router)</strong>', '<strong>IP (IPv4/IPv6), ICMP, IGMP, ARP, RARP, OSPF, BGP, MPLS, RIP, EIGRP</strong>', '<strong>Protocols operating at the Network Layer — implemented in both hosts and routers</strong>', '<strong>Logical addressing, routing, forwarding, error reporting</strong>', '<strong>Hosts use IP/ICMP/ARP to send and receive packets; routers additionally run OSPF/BGP to build routing tables</strong>', '<strong>IPv4 · IPv6 · ICMP · IGMP · ARP · RARP · OSPF · BGP · MPLS · RIP · EIGRP</strong>'],
    ...
    },
    {
      ...
      functions: [
        ['Framing','Divides the stream of bits into manageable data units called frames','On every transmission','LLC header placed before payload; MAC trailer after; together bracket the frame content','Without framing the bit stream has no structure','Solves: no structure in raw bit stream'],
        ['Physical Addressing','Adds a header to define sender and/or receiver of the frame','On every frame sent or received','DSAP and SSAP fields identify source and destination service access points','Without addressing receiver cannot route frame to correct handler','Solves: multiple L3 protocols sharing one interface'],
        ['Flow Control','Imposes a flow control mechanism to avoid overwhelming the receiver','In LLC Type 2 mode when receiver buffer fills','RR (Receiver Ready) and RNR (Receiver Not Ready) supervisory frames signal sender to pause or resume','Without flow control a fast sender overflows a slow receiver buffer','Solves: receiver being overwhelmed at L2'],
        ['Error Control','Adds mechanisms to detect and retransmit damaged or lost frames','In LLC Type 2 mode when an I-frame is lost or FCS fails','REJ and SREJ supervisory frames request retransmission of specific frames','Without error control upper layers must handle all retransmission','Solves: frame loss and corruption'],
        ['Access Control','Determines which device has control over the link at any given time','On shared media when multiple devices compete','Token passing or polling mechanisms arbitrate access; only the token holder may transmit','Without access control simultaneous transmissions cause collisions','Solves: multiple devices sharing one medium'],
        ['Link Establishment and Termination','Establishes and terminates the logical link between two nodes','At start and end of LLC Type 2 (connection-oriented) sessions','SABME command establishes connection; DISC command terminates; UA confirms both','Without explicit setup there is no shared state for reliable sequenced delivery','Solves: no connection context for reliable delivery'],
        ['Frame Sequencing','Transmits and receives frames sequentially','During LLC Type 2 data transfer','I-frames carry send sequence number N(S) and receive sequence number N(R); receiver reorders if needed','Without sequencing frames delivered out of order corrupt upper-layer data','Solves: out-of-order delivery'],
        ['Frame Acknowledgment','Provides and expects frame acknowledgments','After each I-frame in LLC Type 2 mode','RR supervisory frame carries N(R) acknowledging all frames up to N(R)-1; sender retransmits unacknowledged frames after timeout','Without acknowledgment sender cannot know if frame arrived','Solves: silent frame loss'],
        ['Frame Synchronisation','Defines how the receiver detects where each frame begins and ends in a continuous bit stream — four distinct techniques used across different protocols','At the physical/MAC boundary on every received frame across all link-layer technologies','<table style="width:100%;border-collapse:collapse;font-size:11px"><thead><tr><th style="padding:4px 8px;border:1px solid rgba(201,123,247,0.3);color:#c97bf7;text-align:left">Technique</th><th style="padding:4px 8px;border:1px solid rgba(201,123,247,0.3);color:#c97bf7;text-align:left">How it works</th><th style="padding:4px 8px;border:1px solid rgba(201,123,247,0.3);color:#c97bf7;text-align:left">Used in</th><th style="padding:4px 8px;border:1px solid rgba(201,123,247,0.3);color:#c97bf7;text-align:left">Weakness</th></tr></thead><tbody><tr><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2);color:#c97bf7;white-space:nowrap"><strong>Time Based</strong></td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">A master clock divides time into fixed slots; each device transmits only in its assigned slot; receiver locates frame boundaries purely by clock timing</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">SONET/SDH, TDM, Token Ring</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">Requires precise clock synchronisation across all nodes; clock drift causes misalignment</td></tr><tr><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2);color:#c97bf7;white-space:nowrap"><strong>Character Counting</strong></td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">Sender writes payload length in the first header field; receiver reads that count and consumes exactly that many bytes, then expects the next frame header</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">DDCMP, early serial protocols</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">A single corrupted count field misaligns all subsequent frames; resynchronisation requires discarding data</td></tr><tr><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2);color:#c97bf7;white-space:nowrap"><strong>Byte Stuffing</strong></td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">Sender inserts an escape byte before any payload byte matching the delimiter (e.g. PPP: 0x7E→0x7D 0x5E, 0x7D→0x7D 0x5D); receiver reverses substitution after stripping delimiters</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">PPP (flag 0x7E), HDLC, SLIP (END 0xC0)</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">Expands frame size unpredictably; worst case doubles size if all bytes match the delimiter</td></tr><tr><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2);color:#c97bf7;white-space:nowrap"><strong>Bit Stuffing</strong></td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">Sender inserts a 0 bit after every run of five consecutive 1s in the payload; receiver removes any 0 following five 1s; flag pattern (six 1s) can only appear as a true delimiter</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">HDLC, CAN bus, USB</td><td style="padding:4px 8px;border:1px solid rgba(201,123,247,0.2)">Adds overhead proportional to data content; bit-level processing required at sender and receiver</td></tr></tbody></table>','Without frame synchronisation, the receiver cannot locate frame boundaries in the bit stream — all data is unreadable regardless of signal quality','Solves: frame boundary detection across all link technologies'],
      ],
      design_issues: [
      ...
      protocols: [
        ['<strong>Data Link Protocols (Host & Router)</strong>', '<strong>Ethernet (802.3), WiFi (802.11), PPP, HDLC, Frame Relay, ATM, LLC (802.2), VLAN (802.1Q), STP, ARP</strong>', '<strong>Protocols operating at the Data Link Layer — implemented in both hosts and routers</strong>', '<strong>Framing, MAC addressing, error detection, medium access control</strong>', '<strong>Hosts and routers both run Data Link protocols to transmit frames on each directly connected link</strong>', '<strong>Ethernet · WiFi · PPP · HDLC · Frame Relay · ATM · LLC · VLAN (802.1Q) · STP · ARP</strong>'],
    ...
    },
    {
      ...
      protocols: [
        ['<strong>Data Link Protocols (Host & Router)</strong>', '<strong>IEEE 802.3 (Ethernet), IEEE 802.11 (WiFi), ARP, STP/RSTP, VLAN (802.1Q), PPP, HDLC</strong>', '<strong>MAC-layer protocols — implemented in NICs of both hosts and routers</strong>', '<strong>Physical addressing, framing, CRC, medium access control</strong>', '<strong>Every host and router NIC implements MAC framing and addressing to put frames on the wire or air</strong>', '<strong>IEEE 802.3 · IEEE 802.11 · ARP · STP/RSTP · VLAN (802.1Q) · PPP · HDLC</strong>'],
    ...
    },
    {
      ...
      protocols: [
        ['<strong>Physical Layer Protocols (Host & Router)</strong>', '<strong>Ethernet PHY, IEEE 802.11 PHY, SONET/SDH, DSL, 5G NR, USB, Bluetooth, NRZ, Manchester, PAM4</strong>', '<strong>Physical-layer standards — implemented in NICs and router line cards of both hosts and routers</strong>', '<strong>Signal encoding, bit synchronisation, medium attachment, multiplexing</strong>', '<strong>Both hosts and routers implement physical layer standards to transmit and receive raw bit streams on each interface</strong>', '<strong>Ethernet PHY · 802.11 PHY · SONET/SDH · DSL · 5G NR · USB · Bluetooth · NRZ · Manchester · PAM4</strong>'],
...
function renderZoom(section, pkt) {
  ...
  if (section === 'data' || section === 'data-1' || section === 'data-2' || section === 'data-3' || section === 'data-4') {
    ...
    html = `
      ...
      <div class="zoom-section">
        ...
        <div style="overflow-x:auto"><table class="detail-table">
          ...
          <tbody>
            <tr><td class="dt-field">Complexity</td><td class="dt-what">UDP is less complex</td><td class="dt-what">TCP is more complex</td></tr>
            <tr><td class="dt-field">Connection</td><td class="dt-what">UDP is connectionless protocol</td><td class="dt-what">TCP is connection-oriented protocol</td></tr>
            <tr><td class="dt-field">Reliability</td><td class="dt-what">It provides unreliable delivery of messages</td><td class="dt-what">It provides reliable delivery of messages</td></tr>
            <tr><td class="dt-field">Function</td><td class="dt-what">By using this protocol one program can send a load of packets to another and that would be the end of the relationship</td><td class="dt-what">As a message makes its way across the internet from one computer to another — this is connection based</td></tr>
            <tr><td class="dt-field">Which layer they exist</td><td class="dt-what">Transport layer</td><td class="dt-what">Transport layer</td></tr>
            <tr><td class="dt-field">Flow controlling</td><td class="dt-what">UDP has no flow control</td><td class="dt-what">TCP has flow control</td></tr>
            <tr><td class="dt-field">Overhead</td><td class="dt-what">Overhead is very low</td><td class="dt-what">Overhead is low</td></tr>
            <tr><td class="dt-field">Which is powerful</td><td class="dt-what">UDP is less powerful</td><td class="dt-what">TCP is more powerful</td></tr>
            <tr><td class="dt-field">Header size</td><td class="dt-what">8 bytes fixed</td><td class="dt-what">20–60 bytes variable</td></tr>
    ...
    html = `
      ...
      <div class="zoom-section">
        <div class="zoom-section-title">TCP vs UDP</div>
        <div style="overflow-x:auto"><table class="detail-table">
          <thead><tr><th>Feature</th><th>TCP</th><th>UDP</th></tr></thead>
          <tbody>
            <tr><td class="dt-field">Complexity</td><td class="dt-what">TCP is more complex</td><td class="dt-what">UDP is less complex</td></tr>
            <tr><td class="dt-field">Connection</td><td class="dt-what">TCP is connection-oriented protocol</td><td class="dt-what">UDP is connectionless protocol</td></tr>
            <tr><td class="dt-field">Reliability</td><td class="dt-what">It provides reliable delivery of messages</td><td class="dt-what">It provides unreliable delivery of messages</td></tr>
            <tr><td class="dt-field">Function</td><td class="dt-what">As a message makes its way across the internet from one computer to another — this is connection based</td><td class="dt-what">By using this protocol one program can send a load of packets to another and that would be the end of the relationship</td></tr>
            <tr><td class="dt-field">Which layer they exist</td><td class="dt-what">Transport layer</td><td class="dt-what">Transport layer</td></tr>
            <tr><td class="dt-field">Flow controlling</td><td class="dt-what">TCP has flow control</td><td class="dt-what">UDP has no flow control</td></tr>
            <tr><td class="dt-field">Overhead</td><td class="dt-what">Overhead is low</td><td class="dt-what">Overhead is very low</td></tr>
            <tr><td class="dt-field">Which is powerful</td><td class="dt-what">TCP is more powerful</td><td class="dt-what">UDP is less powerful</td></tr>
            <tr><td class="dt-field">Header size</td><td class="dt-what">20–60 bytes variable</td><td class="dt-what">8 bytes fixed</td></tr>
            <tr><td class="dt-field">Ordering</td><td class="dt-what">Yes — sequence numbers enforce order</td><td class="dt-what">No — datagrams may arrive out of order</td></tr>
            <tr><td class="dt-field">Congestion control</td><td class="dt-what">Slow start, AIMD, BBR</td><td class="dt-what">None — sender's problem</td></tr>
            <tr><td class="dt-field">Speed</td><td class="dt-what">Slower — ACKs, handshake, retransmit</td><td class="dt-what">Fast — minimal overhead</td></tr>
            <tr><td class="dt-field">Use cases</td><td class="dt-what">HTTP, FTP, SMTP, SSH, database queries</td><td class="dt-what">DNS, VoIP, video, gaming, DHCP, SNMP</td></tr>
            <tr><td class="dt-field">Broadcast/Multicast</td><td class="dt-what">Not supported</td><td class="dt-what">Supported</td></tr>
          </tbody>
        </table></div>
      </div>`;
  }
    ...
    const ipDiffTable = `<div class="zoom-section"><div class="zoom-section-title">Key differences — IPv4 vs IPv6</div><div style="overflow-x:auto"><table class="detail-table"><thead><tr><th>#</th><th>Feature</th><th>IPv4</th><th>IPv6</th></tr></thead><tbody><tr><td class="dt-field">1</td><td class="dt-what">Address length</td><td class="dt-what">Source and destination addresses are 32 bits (4 bytes) in length</td><td class="dt-what">Source and destination addresses are 128 bits (16 bytes) in length</td></tr><tr><td class="dt-field">2</td><td class="dt-what">Broadcast</td><td class="dt-what">Uses broadcast addresses to send traffic to all nodes on a subnet (e.g. 255.255.255.255)</td><td class="dt-what">There are no IPv6 broadcast addresses — multicast scoped addresses are used instead</td></tr><tr><td class="dt-field">3</td><td class="dt-what">Fragmentation</td><td class="dt-what">Fragmentation is supported at originating hosts and intermediate routers</td><td class="dt-what">Fragmentation is not supported at routers — source host only via Fragment extension header</td></tr><tr><td class="dt-field">4</td><td class="dt-what">Header checksum</td><td class="dt-what">IP header includes a checksum — recomputed by every router at every hop</td><td class="dt-what">IP header does not include a checksum — L2 (Ethernet CRC) and L4 (TCP/UDP) handle error detection</td></tr><tr><td class="dt-field">5</td><td class="dt-what">Options / Extensions</td><td class="dt-what">IP header includes options — making header variable length (20–60 bytes)</td><td class="dt-what">All optional data is moved to IPv6 extension headers — base header is always fixed 40 bytes</td></tr><tr><td class="dt-field">6</td><td class="dt-what">IPsec</td><td class="dt-what">IPsec support is optional</td><td class="dt-what">IPsec support is required in a full IPv6 implementation</td></tr><tr><td class="dt-field">7</td><td class="dt-what">QoS / Payload identification</td><td class="dt-what">No identification of payload for QoS handling by routers is present within the IPv4 header</td><td class="dt-what">Payload identification for QoS handling by routers is included in the IPv6 header using the Flow Label field</td></tr><tr><td class="dt-field">8</td><td class="dt-what">Address representation</td><td class="dt-what">IP address represented in decimal number system</td><td class="dt-what">IP address is represented in hexadecimal number system</td></tr><tr><td class="dt-field">9</td><td class="dt-what">Separator</td><td class="dt-what">Dot ( . ) used as separator — e.g. 192.168.1.1</td><td class="dt-what">Colon ( : ) used as separator — e.g. 2001:0db8::1</td></tr><tr><td class="dt-field">10</td><td class="dt-what">Header size</td><td class="dt-what">20–60 bytes (variable due to options)</td><td class="dt-what">40 bytes fixed</td></tr><tr><td class="dt-field">11</td><td class="dt-what">NAT</td><td class="dt-what">Required due to address exhaustion</td><td class="dt-what">Not needed — every device gets a globally unique address; end-to-end connectivity restored</td></tr><tr><td class="dt-field">12</td><td class="dt-what">Auto-configuration</td><td class="dt-what">DHCP</td><td class="dt-what">SLAAC (Stateless Address Autoconfiguration) — no DHCP server needed; DHCPv6 also supported</td></tr><tr><td class="dt-field">13</td><td class="dt-what">Address resolution</td><td class="dt-what">ARP — broadcast based</td><td class="dt-what">NDP / ICMPv6 Neighbor Discovery — multicast based, no broadcast</td></tr></tbody></table></div></div>`;
    ...
    if (STATE.ipVersion === 4) {
      html = `
        ...
        <div class="zoom-section">
          ...
          ${hdrGrid([
            [['Version','4',0.5],['IHL','5 (20B)',0.5],['DSCP/TOS + ECN','0x00 (+) 00',1],['Total Length', ip.ipLen, 2]],
        ...
        <div class="zoom-section">
          ...
          ${detailTable([
            ...
            ['DSCP (TOS)', '0x00', '6 bits', 'Differentiated Services Code Point — QoS marking (Type of Service)', 'Priority queuing at routers for latency-sensitive traffic', 'Set by sending app or OS; routers read it to select queue', [['0x00 (CS0)','Best-effort default'],['0x28 (CS5)','Voice-grade priority'],['0x2E (EF)','Expedited Forwarding — guaranteed low latency']]],
            ...
            ['Reserved', '0', '1 bit', 'Must be zero — reserved for future use by the IETF', 'Allows future protocol extensions without changing the header structure', 'RFC 791 mandates senders set to 0; receivers must ignore it', [['0','Normal — required value'],['1','Invalid; famously claimed by RFC 3514 (April Fools) as the "Evil Bit" to mark malicious packets']]],
      ...
      <div class="zoom-section">
        <div class="zoom-section-title">Why IPv6? — Design Goals</div>
        <ul style="margin:0;padding:0 0 0 18px;display:flex;flex-direction:column;gap:7px;font-size:13px;line-height:1.6;color:var(--text)">
          <li><span style="color:#f5a623;font-weight:600">Support billions of hosts</span> — 128-bit address space provides 340 undecillion (3.4×10³⁸) unique addresses, permanently solving IPv4 exhaustion without NAT</li>
          <li><span style="color:#f5a623;font-weight:600">Reduce routing table size</span> — hierarchical addressing and aggregation-friendly allocation by RIRs keeps global BGP tables smaller than a naive flat IPv6 deployment would produce</li>
          <li><span style="color:#f5a623;font-weight:600">Simplify the protocol</span> — fixed 40-byte header with no options field, no header checksum, and no router fragmentation; routers process headers faster at line rate</li>
          <li><span style="color:#f5a623;font-weight:600">Better security</span> — IPsec (AH + ESP) support is mandatory in the spec; every IPv6 implementation must support authentication and encryption, not treat it as optional</li>
          <li><span style="color:#f5a623;font-weight:600">More attention to type of service</span> — Traffic Class (DSCP + ECN) and Flow Label fields built into the fixed header enable fine-grained QoS and per-flow handling without options</li>
          <li><span style="color:#f5a623;font-weight:600">Aid multicasting with scopes</span> — multicast addresses embed a scope field (link-local, site-local, global) so routers know exactly how far to forward multicast traffic; broadcast is eliminated entirely</li>
          <li><span style="color:#f5a623;font-weight:600">Host mobility without address change</span> — Mobile IPv6 (RFC 6275) allows a host to roam between networks while keeping its home address, using binding updates to redirect traffic transparently</li>
          <li><span style="color:#f5a623;font-weight:600">Allow future evolution</span> — extension headers replace the rigid IPv4 options field; new header types can be defined without changing the base header; unrecognised extension headers are skipped by routers</li>
          <li><span style="color:#f5a623;font-weight:600">Coexist with IPv4 for years</span> — dual-stack (host runs both), tunnelling (IPv6 in IPv4 packets: 6in4, Teredo, ISATAP), and translation (NAT64/DNS64) mechanisms allow gradual migration without a hard cutover</li>
        </ul>
      </div>
        <div class="zoom-section">
  ...
  const TCPIP_INFO = [
    ...
    },
    {
      ...
      protocols: [
        ...
        ['ARP','Address Resolution Protocol','IP→MAC resolution (RFC 826)','Broadcast "Who has IP x?"; reply with MAC; cached in ARP table'],
        ['RARP','Reverse ARP','MAC→IP resolution (legacy) (RFC 903)','Diskless workstations used RARP to get IP; superseded by BOOTP then DHCP'],
    ...
    },
    {
      ...
      protocols: [
        ...
        ['ARP','Address Resolution Protocol','L3→L2 address mapping (RFC 826)','Sits between Internet and Network Access; broadcast-based; gratuitous ARP for IP conflict detection'],
